...
def job_config = [
        job: [
                name: "CAI_1121_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1669048866425"
        ]
]
...
def customLib = library identifier: 'CAI_1121_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/liucaissr/CAI_1121_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.CAI_1121.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "liucaissr", "CAI_1121", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("CAI_1121_Pipeline_develop");

job.with {

    authenticationToken('BgSxRFe45IewPg4dCCEtYxEuTwPokMrK')
